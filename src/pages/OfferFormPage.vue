...
<template>
  <form class="space-y-4 pb-20 lg:pb-0" @submit.prevent="save">
    ...
    <div class="grid gap-4 lg:grid-cols-[minmax(0,1fr)_20rem] lg:items-start">
      ...
      <aside class="space-y-4 lg:sticky lg:top-20"><Card><template #header><h2 class="text-xs font-semibold uppercase tracking-[0.08em] text-slate-500">Resumo</h2></template><dl class="space-y-2 text-sm"><div class="flex justify-between gap-3"><dt>Preço base</dt><dd class="font-medium text-slate-800">{{ formatCurrency(Number(basePrice) || 0) }}</dd></div><div class="flex justify-between gap-3"><dt>Componentes</dt><dd class="font-medium text-slate-800">{{ components.length }}</dd></div><div class="flex justify-between gap-3"><dt>Grupos</dt><dd class="font-medium text-slate-800">{{ choiceGroups.length }}</dd></div><div class="flex justify-between gap-3"><dt>Adicionais</dt><dd><Badge variant="neutral">{{ allowedAddonIds.length }}</Badge></dd></div><div class="flex justify-between gap-3"><dt>Status</dt><dd><Badge :variant="active ? 'success' : 'neutral'">{{ active ? 'Ativa' : 'Inativa' }}</Badge></dd></div></dl><template #footer><Button type="submit" class="w-full" :loading="saving" :disabled="props.mode === 'edit' && !existing">{{ props.mode === 'edit' ? 'Salvar alterações' : 'Salvar oferta' }}</Button></template></Card></aside>
    </div>
